...
**🤖 Agent / 脚本获取原始 PDF**（Web Chat AI 请使用下方 AI Reading Path）：每个 PDF 的原始文件托管在 `raw.githubusercontent.com`，无需 JavaScript 即可经 HTTP 获取，返回真实 PDF 二进制（已开启 CORS、支持 Range 分段请求）。此入口适用于**真实具备 HTTP 二进制下载 + 本地 PDF parser 的 Agent / 脚本环境**（如带 PyMuPDF 的自动化脚本），不是普通网页聊天 AI 的默认正文入口：
...
**Web Chat AI 入口（静态 HTML，直接打开）**：

[https://yeblue1029.github.io/puzhong-DSP28335-learning-materials/ai/](https://yeblue1029.github.io/puzhong-DSP28335-learning-materials/ai/)

网页聊天 AI 请**沿该页面中的真实 HTML 链接逐页点击导航**：
文档列表 → 文档页面（landing page）→ full.txt / full.html / Pages 目录 / Blocks 目录 → 单页文本。
无需自行拼接 URL，也无需先读取 JSON。

**机器入口（JSON，供支持任意 HTTP fetch 的 Agent / Script / AI 使用）**：
...
Web Chat AI  → /ai/ 静态 HTML → 文档页面 → full.txt / pages / blocks（沿真实链接）
Agent/Script → /ai/index.json 或 original_raw_url（原始 raw PDF）
```

⚠️ **Web Chat AI 不再默认 raw PDF 优先**：请先读 [`AI_ACCESS.md`](AI_ACCESS.md)
（仓库根目录，AI 访问契约：HTML 链接导航规则 / 文本来源等级 / OCR 警示 / 失败规则），
再进入上方的 AI Reading Path。OCR 页（`TEXT_SOURCE: ocr`）为机器识别文本，不是"原文等价物"，
关键结论应回原始 PDF 核验。
