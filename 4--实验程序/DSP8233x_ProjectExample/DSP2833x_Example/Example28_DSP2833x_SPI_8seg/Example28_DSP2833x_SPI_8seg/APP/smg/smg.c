...
void SMGBitSelectionGPIO_Init(void)
{
	EALLOW;
	SysCtrlRegs.PCLKCR3.bit.GPIOINENCLK = 1;// 开启GPIO时钟
	//SMG位选控制端口配置
...
void SMGSPIA_Init(void)
{
	SMGBitSelectionGPIO_Init();
	SPIA_Init();
}

void SMGSPIA_DisplayInt(Uint16 num)
{
	unsigned char buf[4];
	unsigned char i=0;
...
void SMGSPIA_DisplayFloat(float num,unsigned char dotnum)
{
	unsigned char buf[4];
	unsigned char i;
	Uint16 value=0;
